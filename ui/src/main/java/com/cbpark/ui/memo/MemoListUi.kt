...
import com.cbpark.ui.card.SwipeToRevealCard
...
fun MemoListUi(
  ...
  memoDeleteEvent: (memo: Memo) -> Unit
) {
  LazyColumn(
    ...
    itemsIndexed(memos) { index, memo ->
      SwipeToRevealCard(
        cardContent = { MemoUiContent(memo = memo) },
        onClick = { memoClickEvent(memo) },
        onDelete = { memoDeleteEvent(memo) }
      )
